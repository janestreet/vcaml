...
let list_wins ?(here = Stdlib.Lexing.dummy_pos) client t =
...
let get_win ?(here = Stdlib.Lexing.dummy_pos) client t =
  Nvim_internal.nvim_tabpage_get_win ~tabpage:t |> run ~here client
;;

let set_win ?(here = Stdlib.Lexing.dummy_pos) client t win =
