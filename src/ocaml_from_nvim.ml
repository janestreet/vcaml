...
let register_request_async ?(here = Stdlib.Lexing.dummy_pos) client ~name ~type_ ~f =
...
let subscribe_to_broadcast ?(here = Stdlib.Lexing.dummy_pos) client ~name =
  Nvim_internal.nvim_subscribe ~event:name |> run ~here client
;;

let unsubscribe_from_broadcast ?(here = Stdlib.Lexing.dummy_pos) client ~name =
