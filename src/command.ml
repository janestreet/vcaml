...
let delete ?(here = Stdlib.Lexing.dummy_pos) client name ~scope =
  let query =
    match scope with
...
let exec ?(here = Stdlib.Lexing.dummy_pos) client =
  exec_internal
    ~output:false
...
let exec_and_capture_output ?(here = Stdlib.Lexing.dummy_pos) client =
...
let user_defined_commands ?(here = Stdlib.Lexing.dummy_pos) client ~scope =
  let query =
    match scope with
...
module Fast = struct
  let parse ?(here = Stdlib.Lexing.dummy_pos) client str =
    (* [opts] is not used by this version of Neovim, but may be used in the future. If we
       expose it, we should do so in a typeful way rather than asking the user to build
       [Msgpack.t] values. *)
