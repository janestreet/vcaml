...
module Private = struct
    ...
    type blocking =
      run_in_background:
        (?here:Stdlib.Lexing.position
         -> ([ `asynchronous ] T.t -> unit Deferred.Or_error.t)
         -> unit)
      -> [ `blocking ] T.t
      -> Msgpack.t list
  ...
  and T : sig
    ...
    type 'kind t =
      ...
      ; register_request_async :
          ?here:Stdlib.Lexing.position -> string -> f:Callbacks.async -> unit
      ...
      ; notify_nvim_of_error : ?here:Stdlib.Lexing.position -> Error.t -> unit Deferred.t
  ...
  module Not_connected = struct
    ...
    let register_request_async ?(here = Stdlib.Lexing.dummy_pos) t ~name ~f =
  ...
  let nvim_list_chans ?(here = Stdlib.Lexing.dummy_pos) t =
  ...
  let channel_info ?(here = Stdlib.Lexing.dummy_pos) ?(chan = 0) t =
...
let connect
  ...
  let notify_nvim_of_error ?(here = Stdlib.Lexing.dummy_pos) error =
    ...
    |> Deferred.ignore_m
  in
  let try_with ?(here = Stdlib.Lexing.dummy_pos) f =
  ...
  and run_in_background ?(here = Stdlib.Lexing.dummy_pos) f =
    don't_wait_for
  ...
  and register_request_async ?(here = Stdlib.Lexing.dummy_pos) name ~f =
...
let block_nvim' ?(here = Stdlib.Lexing.dummy_pos) t ~f =
