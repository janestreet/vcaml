...
let get_buf ?(here = Stdlib.Lexing.dummy_pos) client t =
  Nvim_internal.nvim_win_get_buf ~window:t |> run ~here client
;;

let set_buf ?(here = Stdlib.Lexing.dummy_pos) client t ~buffer =
...
let close
  ...
  client
  t
  ~when_this_is_the_buffer's_last_window
  =
...
let get_height ?(here = Stdlib.Lexing.dummy_pos) client t =
  Nvim_internal.nvim_win_get_height ~window:t |> run ~here client
;;

let set_height ?(here = Stdlib.Lexing.dummy_pos) client t ~height =
  Nvim_internal.nvim_win_set_height ~window:t ~height |> run ~here client
;;

let get_width ?(here = Stdlib.Lexing.dummy_pos) client t =
  Nvim_internal.nvim_win_get_width ~window:t |> run ~here client
;;

let set_width ?(here = Stdlib.Lexing.dummy_pos) client t ~width =
  Nvim_internal.nvim_win_set_width ~window:t ~width |> run ~here client
;;

let get_cursor ?(here = Stdlib.Lexing.dummy_pos) client t =
...
let get_tab ?(here = Stdlib.Lexing.dummy_pos) client t =
  Nvim_internal.nvim_win_get_tabpage ~window:t |> run ~here client
;;

let get_number ?(here = Stdlib.Lexing.dummy_pos) client t =
  Nvim_internal.nvim_win_get_number ~window:t |> run ~here client
;;

let get_position ?(here = Stdlib.Lexing.dummy_pos) client t =
...
let get_config ?(here = Stdlib.Lexing.dummy_pos) client t =
  Nvim_internal.nvim_win_get_config ~window:t
  |> map_witness ~f:Config.of_msgpack_map
  |> run ~here client
;;

let set_config ?(here = Stdlib.Lexing.dummy_pos) client t config =
...
module Option = struct
  ...
  let get ?(here = Stdlib.Lexing.dummy_pos) client window t =
  ...
  let set ?(here = Stdlib.Lexing.dummy_pos) client window t value =
  ...
  let get_for_current_buffer_in_window ?(here = Stdlib.Lexing.dummy_pos) client window t =
    Nvim_internal.nvim_get_option_value
  ...
  let set_for_current_buffer_in_window
    ?(here = Stdlib.Lexing.dummy_pos)
    client
    window
    t
  ...
  let get_global ?(here = Stdlib.Lexing.dummy_pos) client window t =
    Nvim_internal.nvim_get_option_value
      ~name:(Per_buffer.to_string t)
      ~opts:
  ...
  let set_global ?(here = Stdlib.Lexing.dummy_pos) client window t value =
    Nvim_internal.nvim_set_option_value
      ~name:(Per_buffer.to_string t)
      ~value:(Per_buffer.to_msgpack t value)
      ~opts:
  ...
  let set_default ?(here = Stdlib.Lexing.dummy_pos) client t value =
    set_global ~here client Current t value
  ;;

  let get_default ?(here = Stdlib.Lexing.dummy_pos) client t =
    get_global ~here client Current t
  ;;
