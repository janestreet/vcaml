...
module Group = struct
  ...
  let create ?(here = Stdlib.Lexing.dummy_pos) client ?clear_if_exists name =
    let opts =
  ...
  let delete ?(here = Stdlib.Lexing.dummy_pos) client t =
...
let get ?(here = Stdlib.Lexing.dummy_pos) client ?group ?events ?patterns_or_buffer () =
  let opts =
...
let delete ?(here = Stdlib.Lexing.dummy_pos) client id =
  Nvim_internal.nvim_del_autocmd ~id |> run ~here client
;;

let clear ?(here = Stdlib.Lexing.dummy_pos) client ?patterns_or_buffer () ~group ~events =
  let opts =
    let event = events_to_msgpack events in
    let group =
      match group with
