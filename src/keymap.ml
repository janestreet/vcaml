...
let get ?(here = Stdlib.Lexing.dummy_pos) client ~scope ~mode =
  let query =
    match scope with
...
let set ?(here = Stdlib.Lexing.dummy_pos) client =
  set_internal ~return_type:Nil ~expr:false ~here client ?replace_keycodes:None
;;

let set_expr ?(here = Stdlib.Lexing.dummy_pos) client =
  set_internal ~return_type:String ~expr:true ~here client
;;

let unset ?(here = Stdlib.Lexing.dummy_pos) client ~scope ~lhs ~mode =
  let query =
    match scope with
