...
let channels ?(here = Stdlib.Lexing.dummy_pos) client =
  let client = Type_equal.conv Client.Private.eq client in
  Client.Private.nvim_list_chans ~here client
;;

let exec_viml ?(here = Stdlib.Lexing.dummy_pos) client code =
...
let exec_viml_and_capture_output ?(here = Stdlib.Lexing.dummy_pos) client code =
...
let exec_lua ?(here = Stdlib.Lexing.dummy_pos) client code =
  Nvim_internal.nvim_exec_lua ~code ~args:[]
  |> map_witness ~f:(Type.of_msgpack Nil)
  |> run ~here client
;;

let list_bufs ?(here = Stdlib.Lexing.dummy_pos) client =
  Nvim_internal.nvim_list_bufs |> run ~here client
;;

let get_channel_info ?(here = Stdlib.Lexing.dummy_pos) client chan =
  Nvim_internal.nvim_get_chan_info ~chan
  |> map_witness ~f:Channel_info.of_msgpack_map
  |> run ~here client
;;

let eval_viml_expression ?(here = Stdlib.Lexing.dummy_pos) client expr ~result_type =
...
let call_function ?(here = Stdlib.Lexing.dummy_pos) client ~name ~type_ =
...
let get_current_buf ?(here = Stdlib.Lexing.dummy_pos) client =
  Nvim_internal.nvim_get_current_buf |> run ~here client
;;

let set_current_buf ?(here = Stdlib.Lexing.dummy_pos) client buffer =
  Nvim_internal.nvim_set_current_buf ~buffer |> run ~here client
;;

let list_tabs ?(here = Stdlib.Lexing.dummy_pos) client =
  Nvim_internal.nvim_list_tabpages |> run ~here client
;;

let get_current_tab ?(here = Stdlib.Lexing.dummy_pos) client =
  Nvim_internal.nvim_get_current_tabpage |> run ~here client
;;

let set_current_tab ?(here = Stdlib.Lexing.dummy_pos) client tabpage =
...
let get_current_win ?(here = Stdlib.Lexing.dummy_pos) client =
  Nvim_internal.nvim_get_current_win |> run ~here client
;;

let set_current_win ?(here = Stdlib.Lexing.dummy_pos) client window =
  Nvim_internal.nvim_set_current_win ~window |> run ~here client
;;

let list_wins ?(here = Stdlib.Lexing.dummy_pos) client =
  Nvim_internal.nvim_list_wins |> run ~here client
;;

type keys_with_replaced_keycodes = string

let replace_termcodes ?(here = Stdlib.Lexing.dummy_pos) client str ~replace_keycodes =
...
let feedkeys ?(here = Stdlib.Lexing.dummy_pos) client keys ~mode =
...
let get_color_by_name ?(here = Stdlib.Lexing.dummy_pos) client name =
...
let get_color_map ?(here = Stdlib.Lexing.dummy_pos) client =
...
let get_hl_id_by_name ?(here = Stdlib.Lexing.dummy_pos) client name =
...
let get_var ?(here = Stdlib.Lexing.dummy_pos) client name ~type_ =
  Nvim_internal.nvim_get_var ~name
  |> map_witness ~f:(Type.of_msgpack type_)
  |> run ~here client
;;

let set_var ?(here = Stdlib.Lexing.dummy_pos) client name ~type_ ~value =
  let value = Type.to_msgpack type_ value in
  Nvim_internal.nvim_set_var ~name ~value |> run ~here client
;;

let delete_var ?(here = Stdlib.Lexing.dummy_pos) client name =
  Nvim_internal.nvim_del_var ~name |> run ~here client
;;

let get_vvar ?(here = Stdlib.Lexing.dummy_pos) client name ~type_ =
  Nvim_internal.nvim_get_vvar ~name
  |> map_witness ~f:(Type.of_msgpack type_)
  |> run ~here client
;;

let set_vvar ?(here = Stdlib.Lexing.dummy_pos) client name ~type_ ~value =
  let value = Type.to_msgpack type_ value in
  Nvim_internal.nvim_set_vvar ~name ~value |> run ~here client
;;

let list_runtime_paths ?(here = Stdlib.Lexing.dummy_pos) client =
  Nvim_internal.nvim_list_runtime_paths |> run ~here client
;;

let out_write ?(here = Stdlib.Lexing.dummy_pos) client str =
  Nvim_internal.nvim_out_write ~str |> run ~here client
;;

(* For some reason this isn't a supported API function like [err_writeln]. *)
let out_writeln ?(here = Stdlib.Lexing.dummy_pos) client str =
  Nvim_internal.nvim_out_write ~str:(str ^ "\n") |> run ~here client
;;

let err_write ?(here = Stdlib.Lexing.dummy_pos) client str =
  Nvim_internal.nvim_err_write ~str |> run ~here client
;;

let err_writeln ?(here = Stdlib.Lexing.dummy_pos) client str =
  Nvim_internal.nvim_err_writeln ~str |> run ~here client
;;

let echo ?(here = Stdlib.Lexing.dummy_pos) client message ~add_to_history =
...
module Fast = struct
  let get_mode ?(here = Stdlib.Lexing.dummy_pos) client =
  ...
  let input ?(here = Stdlib.Lexing.dummy_pos) client keys =
  ...
  let find_runtime_file_matching ?(here = Stdlib.Lexing.dummy_pos) client ~pattern =
  ...
  let all_runtime_files_matching ?(here = Stdlib.Lexing.dummy_pos) client ~pattern =
    Nvim_internal.nvim_get_runtime_file ~name:pattern ~all:true |> run ~here client
  ;;
end

let paste ?(here = Stdlib.Lexing.dummy_pos) client data =
...
let paste_stream ?(here = Stdlib.Lexing.dummy_pos) client =
  let open Async in
...
let put ?(here = Stdlib.Lexing.dummy_pos) client ?text_mode lines ~where ~place_cursor =
  let type_ =
...
(* The <CR> is buffered and then used to reply to the prompt. The side-effect of this
   ...
   waiting). *)
let echo_in_rpcrequest ?(here = Stdlib.Lexing.dummy_pos) client message =
...
let notify ?(here = Stdlib.Lexing.dummy_pos) client log_level message =
  (* [opts] is not used by this version of Neovim, but may be used in the future. If we
     expose it, we should do so in a typeful way rather than asking the user to build
     [Msgpack.t] values. *)
...
let send_to_channel ?(here = Stdlib.Lexing.dummy_pos) client ~channel data =
  Nvim_internal.nvim_chan_send ~chan:channel ~data |> run ~here client
;;

let get_current_line ?(here = Stdlib.Lexing.dummy_pos) client =
  Nvim_internal.nvim_get_current_line
  |> map_witness ~f:(fun line -> Ok (String.Utf8.of_string_unchecked line))
  |> run ~here client
;;

let set_current_line ?(here = Stdlib.Lexing.dummy_pos) client line =
  Nvim_internal.nvim_set_current_line ~line |> run ~here client
;;

let delete_current_line ?(here = Stdlib.Lexing.dummy_pos) client =
...
let get_context ?(here = Stdlib.Lexing.dummy_pos) client context_types =
  let opts =
    let context_types =
      context_types
      |> Set.to_list
...
let load_context ?(here = Stdlib.Lexing.dummy_pos) client context =
...
let get_mark ?(here = Stdlib.Lexing.dummy_pos) client sym =
  (* [opts] is not used by this version of Neovim, but may be used in the future. If we
     expose it, we should do so in a typeful way rather than asking the user to build
     [Msgpack.t] values. *)
...
let delete_mark ?(here = Stdlib.Lexing.dummy_pos) client sym =
...
let get_display_width_of_text ?(here = Stdlib.Lexing.dummy_pos) client text =
  Nvim_internal.nvim_strwidth ~text |> run ~here client
;;

let set_current_dir ?(here = Stdlib.Lexing.dummy_pos) client dir =
...
module Option = struct
  ...
  let get (type a) ?(here = Stdlib.Lexing.dummy_pos) client (t : a t)
  ...
  let set (type a) ?(here = Stdlib.Lexing.dummy_pos) client (t : a t) (value : a)
  ...
  let get_dynamic_info (type a) ?(here = Stdlib.Lexing.dummy_pos) client (t : a t) =
