...
let get_changedtick ?(here = Stdlib.Lexing.dummy_pos) client t =
...
let get_name ?(here = Stdlib.Lexing.dummy_pos) client t =
  Nvim_internal.nvim_buf_get_name ~buffer:t |> run ~here client
;;

let set_name ?(here = Stdlib.Lexing.dummy_pos) client t name =
  Nvim_internal.nvim_buf_set_name ~buffer:t ~name |> run ~here client
;;

let get_lines ?(here = Stdlib.Lexing.dummy_pos) client t ~start ~end_ ~strict_indexing =
...
let create ?(here = Stdlib.Lexing.dummy_pos) client ~listed ~scratch =
...
let find_by_name_or_create ?(here = Stdlib.Lexing.dummy_pos) client name =
...
let loaded ?(here = Stdlib.Lexing.dummy_pos) client t =
  Nvim_internal.nvim_buf_is_loaded ~buffer:t |> run ~here client
;;

let exists ?(here = Stdlib.Lexing.dummy_pos) client t =
  Nvim_internal.nvim_buf_is_valid ~buffer:t |> run ~here client
;;

let get_buffer ?(here = Stdlib.Lexing.dummy_pos) client or_current =
...
let subscribe ?(here = Stdlib.Lexing.dummy_pos) client ?(send_buffer = true) t =
...
let set_mark ?(here = Stdlib.Lexing.dummy_pos) client ?changedtick t mark =
  (* [opts] is not used by this version of Neovim, but may be used in the future. If we
     expose it, we should do so in a typeful way rather than asking the user to build
     [Msgpack.t] values. *)
...
let delete_mark ?(here = Stdlib.Lexing.dummy_pos) client t sym =
...
let line_count ?(here = Stdlib.Lexing.dummy_pos) client t =
  Nvim_internal.nvim_buf_line_count ~buffer:t |> run_and_get_changedtick here client t
;;

let get_byte_offset_of_line ?(here = Stdlib.Lexing.dummy_pos) client t ~line:index =
...
module Untested = struct
  ...
  let delete_extmark ?(here = Stdlib.Lexing.dummy_pos) client extmark =
  ...
  let all_extmarks ?(here = Stdlib.Lexing.dummy_pos) client =
  ...
  let all_extmarks_with_details ?(here = Stdlib.Lexing.dummy_pos) client =
    all_extmarks_internal
      ~map_witness_f:(fun ~buffer ~namespace extmarks ->
        extmarks
        |> List.map ~f:(function
...
module Option = struct
  ...
  let set_local ?(here = Stdlib.Lexing.dummy_pos) client buffer t value =
  ...
  let get_global ?(here = Stdlib.Lexing.dummy_pos) client t =
  ...
  let set_global ?(here = Stdlib.Lexing.dummy_pos) client t value =
    Nvim_internal.nvim_set_option_value
      ~name:(to_string t)
      ~value:(to_msgpack t value)
      ~opts:(String.Map.singleton "scope" (Msgpack.String "global"))
  ...
  let get (type a g) ?(here = Stdlib.Lexing.dummy_pos) client buffer (t : (a, g) t) =
  ...
  let get_dynamic_info (type a g) ?(here = Stdlib.Lexing.dummy_pos) client (t : (a, g) t) =
...
let open_term ?(here = Stdlib.Lexing.dummy_pos) client t =
  let open Deferred.Or_error.Let_syntax in
