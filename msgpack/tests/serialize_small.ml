...
let%expect_test "fixext1" =
  let obj = Extension { type_id = 0x5; data = Bytes.init 1 ~f:(fun _ -> '\x80') } in
  let s = string_of_t_exn obj in
  print_hex s;
  [%expect {| \xd4\x05\x80 |}]
;;
...
let%expect_test "fixext2" =
  let obj = Extension { type_id = 0x5; data = Bytes.init 2 ~f:(fun _ -> '\x80') } in
  let s = string_of_t_exn obj in
  print_hex s;
...
let%expect_test "fixext4" =
  let obj = Extension { type_id = 0x5; data = Bytes.init 4 ~f:(fun _ -> '\x80') } in
  let s = string_of_t_exn obj in
  print_hex s;
...
let%expect_test "fixext8" =
  let obj = Extension { type_id = 0x5; data = Bytes.init 8 ~f:(fun _ -> '\x80') } in
  let s = string_of_t_exn obj in
  print_hex s;
